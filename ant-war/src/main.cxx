...
int main(int argc, char ** argv)
{
	...
	const int nombreBoids = 50;


	Boid Boids[nombreBoids];

	std::random_device rd;
	std::default_random_engine eng;
	std::uniform_real_distribution<float> rand1(100, 500);
	std::uniform_real_distribution<float> rand2(-10, 10);
	for (int i = 0; i < nombreBoids; ++i) {
		...
		float dx = rand2(eng);
		float dy = rand2(eng);
		Boids[i] = Boid(x, y, dx, dy);
	};

	Boid a = Boid(100, 100, 10, 10);
	a.RenderBoid(g.renderer);
	bool end = false;
	while (not end) {
		SDL_Event event;
		if (SDL_WaitEventTimeout(&event, 20)) {
			...
			for (int i = 0; i < nombreBoids; i++) {
				Boids[i].UpdateBoid(Boids, i, nombreBoids);
				Boids[i].RenderBoid(g.renderer);
			}
